...
class Report extends Request {
    protected function post() {
        ...
        if(Database::existCheck('SELECT * FROM comment_report_request WHERE member_id=? AND comment_id=? AND report_answered=0', [$this->userId, $this->data['commentID']])) {
    ...
    private function reportFunc() {
        $query = Database::executeWithError('INSERT INTO comment_report_request (member_id, comment_id, report_option_id, report_text) VALUES (?,?,?,?)', [$this->userId, $this->data['commentID'], $this->data['reportOptionID'], $this->data['reportText']]);
        if(!$query[0]) {
    ...
    private function hideComment() {
        ...
        $crrID = Database::getRow('SELECT comment_report_request_id FROM comment_report_request WHERE comment_id=? AND member_id=?', [$this->data['commentID'],$this->userId])['comment_report_request_id'];
        Database::execute('INSERT INTO hidden_comment (comment_id, member_id, comment_report_request_id) VALUES(?,?,?)', [$this->data['commentID'],$this->userId, $crrID]);
    }
